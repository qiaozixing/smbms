...
function deleteProvider(obj){
	$.ajax({
		...
		success:function(data){
			...
			}else if($.trim(data.delResult) == $.trim("false")){//删除失败
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/providerDetail.html?id="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/providerModify.html?id="+ obj.attr("proid");
	});
